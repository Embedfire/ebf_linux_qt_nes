...
#define KEY_INPUT_LEFT 30
#define KEY_INPUT_DOWN 31
#define KEY_INPUT_RIGHT 32
#define KEY_INPUT_UP 17

#define KEY_INPUT_A 22
#define KEY_INPUT_B 23
#define KEY_INPUT_C 36
#define KEY_INPUT_D 37
...
static void *InputEventTreadKeyboard(void *pVoid) {
    int fd = 0;
    struct input_event inmyself;
    bool m_bRun = true;

//    fd = open("/dev/input/by-path/platform-ci_hdrc.1-usb-0:1.3:1.0-event-kbd", O_RDWR);
    fd = open("/dev/input/by-path/platform-ci_hdrc.1-usb-0:1.1:1.0-event-kbd", O_RDWR);
    if (fd < 0)
    {
        printf("Open keyboard failed .\n");
    }

    printf("Open keyboard ok\n\n");
    static unsigned char nKeyId = 0;
    ...
    while (m_bRun)
    {
        ...
        {
            nKeyId = inmyself.code;
            // 按键按下
            if (1 == inmyself.value)
            {
                if (KEY_INPUT_UP == nKeyId) {
                    dwKeyPad1 |= 1 << 4;
                }

                if (KEY_INPUT_DOWN == nKeyId) {
                    dwKeyPad1 |= 1 << 5;
                }

                if (KEY_INPUT_LEFT == nKeyId) {
                    dwKeyPad1 |= 1 << 6;
                }

                if (KEY_INPUT_RIGHT == nKeyId) {
                    dwKeyPad1 |= 1 << 7;
                }

                if (KEY_INPUT_A == nKeyId) {
                    dwKeyPad1 |= 1 << 2;
                }

                if (KEY_INPUT_B == nKeyId) {
                    dwKeyPad1 |= 1 << 3;
                }

                if (KEY_INPUT_C == nKeyId) {
                    dwKeyPad1 |= 1 << 0;
                }

                if (KEY_INPUT_D == nKeyId) {
                    dwKeyPad1 |= 1 << 1;
                }

            }
            else if (0 == inmyself.value) {
                if (KEY_INPUT_UP == nKeyId) {
                    dwKeyPad1 &= ~(1 << 4);
                }
...
int InitJoypadInput(void)
{
    ...
    pthread_t tTreadID;
    pthread_create(&tTreadID, NULL, InputEventTreadKeyboard, NULL);
